import type {SingleUserDeviceProcessed} from "../Structures/Sessions.tsx";
...
export default function SessionDevice(
    ...
    return (
        <div key={device.id} className="flex items-center gap-4 p-4 rounded-lg bg-[#0b0f14] border border-gray-800">
            <div className="w-14 h-14 rounded-full bg-linear-to-br from-gray-800 to-gray-900 flex justify-center items-center">
                <img alt={device.device} src={device.icon}/>
            </div>
            ...
            <div className="flex-none">
                <button className="px-3 py-2 text-sm rounded-md bg-red-600 hover:bg-red-700 text-white disabled:opacity-60"
                        onClick={() => revoke(device.id)} disabled={disabled}>
                    Sign out
                </button>
            </div>
        </div>
